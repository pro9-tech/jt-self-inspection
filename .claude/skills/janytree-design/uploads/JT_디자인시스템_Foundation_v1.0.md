...
--jt-font-num: "Figtree", "Pretendard Variable", ui-monospace, sans-serif;
```
...
--jt-bp-mobile:  639px;
--jt-bp-tablet:  1023px;
--jt-bp-desktop: 1439px;
--jt-container-max: 1200px;   /* 본문 최대폭 — 넘으면 좌우 여백 */
--jt-sider-width: 236px;
--jt-sider-collapsed: 64px;    /* 태블릿에서 아이콘만 */
```

### 3-2. 그리드 & 컨테이너

- **12컬럼** 그리드, 거터 24px(desktop)/16px(tablet)/12px(mobile)
- 본문 최대폭 `--jt-container-max`(1200px), 초과 시 가운데 정렬 + 좌우 여백
...
```css
--jt-shadow-1: 0 1px 2px rgba(25,31,40,.06), 0 1px 3px rgba(25,31,40,.04);  /* 카드 */
--jt-shadow-2: 0 2px 8px rgba(25,31,40,.06), 0 1px 3px rgba(25,31,40,.05);  /* 드롭다운 */
--jt-shadow-3: 0 8px 24px rgba(25,31,40,.10), 0 2px 6px rgba(25,31,40,.06); /* 모달 */
--jt-shadow-4: 0 20px 40px rgba(25,31,40,.14), 0 4px 10px rgba(25,31,40,.08); /* 팝오버 큰 것 */
```
...
```css
[data-theme="dark"]{
  --jt-elev-0: #0F172A;   /* 페이지 바탕 (가장 어두움) */
  --jt-elev-1: #1E293B;   /* 카드·패널 */
  --jt-elev-2: #273449;   /* 드롭다운·팝오버·떠 있는 표면 */
  --jt-elev-3: #2E3C52;   /* 모달·시트 */
  --jt-elev-4: #35435B;   /* 최상위(모달 위 팝오버 등) */
  /* 층 구분용 상단 하이라이트(선택) */
  --jt-elev-hairline: inset 0 1px 0 rgba(255,255,255,.06);
  /* 다크 그림자는 더 짙고 은은하게, 보조 수단으로만 */
  --jt-shadow-dark: 0 8px 24px rgba(0,0,0,.36);
}
```
...
2. 고도가 오르면 `--jt-elev-hairline`(상단 흰 실선)을 더해 "빛을 받는 윗면"을 암시.
3. 다크에서 그림자(`--jt-shadow-dark`)는 **분리 보조**로만. 주 신호는 표면 밝기.
4. 라이트/다크에서 컴포넌트는 `--jt-color-bg-elevated` 같은 **의미 이름만** 참조 →
...
:root{                          /* 기본(comfortable) */
  ...
  --jt-density-fs-base: 14px;
  --jt-density-fs-sm:   12px;
  --jt-cell-pad-x: 12px;
  --jt-cell-pad-y: 8px;
  --jt-density-lh: 1.6;
}
[data-density="compact"]{
  --jt-control-height: 30px;       /* 28 아님 — 28은 글자 여백이 부족 */
  --jt-density-fs-base: 13px;      /* 14 → 13 (한 스텝만) */
  --jt-density-fs-sm:   12px;      /* 그대로 — 12px가 본문 최소 바닥 */
  --jt-cell-pad-x: 10px;           /* 좌우 여백 바닥 = 8px, 그 위 */
  --jt-cell-pad-y: 5px;            /* 상하 여백 바닥 = 4px, 그 위 */
  --jt-density-lh: 1.5;            /* 1.45 미만으로 내리지 않음 */
}
```
...
├─ inherits: --jt-control-height, --cell-pad-*, --density-fs-*, --jt-density-lh
├─ compact 허용: yes  (data-density="compact")
├─ 오버라이드 가능: --jt-cell-pad-x, --jt-cell-pad-y  (단, floor 이상)
├─ 오버라이드 금지: --jt-density-fs-base < 12px, --jt-density-lh < 1.45
└─ 숫자열: 항상 --jt-font-num + tabular-nums (밀도 무관)
```
...
### 5-1. 시간(duration)
```css
--jt-dur-instant: 100ms;  /* 상태 즉시(체크·토글) */
--jt-dur-fast:    160ms;  /* 호버·포커스 */
--jt-dur-base:    240ms;  /* 대부분의 전환(드롭다운·탭) */
--jt-dur-slow:    360ms;  /* 모달·시트 진입 */
--jt-dur-page:    480ms;  /* 페이지·큰 레이어 */
```

### 5-2. 이징(easing)
```css
--jt-ease-standard:   cubic-bezier(.4, 0, .2, 1);   /* 기본(감속 진입) */
--jt-ease-decelerate: cubic-bezier(0, 0, .2, 1);    /* 진입(들어옴) */
--jt-ease-accelerate: cubic-bezier(.4, 0, 1, 1);    /* 이탈(나감) */
--jt-ease-emphasized: cubic-bezier(.2, 0, 0, 1);    /* 강조 전환 */
```
...
| 상황 | 모션 |
|---|---|
| 호버 | 배경·색 `--jt-dur-fast` 페이드 |
| 버튼 누름 | `transform: scale(.98)` `--jt-dur-instant` |
| 드롭다운·툴팁 | opacity+`translateY(-4px→0)` `--jt-dur-base` `--jt-ease-decelerate` |
| 모달 | 오버레이 페이드 + 카드 `scale(.98→1)` `--jt-dur-slow` |
