...
```css
/* ── 색 씨앗 ── */
--jt-seed-color-primary: #1F2328;   /* 회사 대표색 = 로고색 = Neutral Charcoal. 전 앱 동일 */
--jt-seed-color-neutral: #1F2328;   /* 뉴트럴 스케일의 기준(=neutral-900=로고색). primary와 동일 */
--jt-seed-color-accent:  #305CDE;   /* 상호작용 강조(링크·포커스·선택) = 9색 blue-500. charcoal은 링크로 부적합해 분리 */
--jt-seed-color-success:  #14A870;  /* 성공·승인·정상 = green-500 */
--jt-seed-color-warning:  #F0B01C;  /* 주의·경고 = yellow-500 (표현: yellow-50 배경+yellow-900 글자) */
--jt-seed-color-error:    #E14B4B;  /* 오류·반려·규격초과 = red-500 */
--jt-seed-color-info:     #305CDE;  /* 정보·안내 (accent=blue-500와 동일값) */

/* ── 크기 씨앗 ── */
--jt-seed-font-size: 14px;   /* 본문 기준. 여기서 타입 스케일 파생 */
--jt-seed-radius:    8px;    /* 기준 모서리. 여기서 라운드 스케일 파생 */
--jt-seed-unit:      4px;    /* 간격 그리드 기본 단위 */
...
```css
--jt-color-primary-bg:     #EEF0F2;  /* = neutral-100 */
--jt-color-primary-border:  #DEE2E6;  /* = neutral-200 */
--jt-color-primary-hover:   #2E343B;  /* = neutral-800 */
--jt-color-primary:         #1F2328;  /* = neutral-900 = 로고색 */
--jt-color-primary-active:  #14171A;  /* = neutral-950 */
--jt-color-primary-text:    #1F2328;  /* = neutral-900 */
```
상호작용 accent(info blue)는 별도 6단:
```css
--jt-color-accent-bg:     #EEF3FE;  /* = blue-50 */
--jt-color-accent-border:  #D6E1FC;  /* = blue-100 */
--jt-color-accent-hover:   #5578E8;  /* = blue-400 */
--jt-color-accent:         #305CDE;  /* = blue-500 = 링크·포커스 */
--jt-color-accent-active:  #2A4EC0;  /* = blue-600 */
--jt-color-accent-text:    #24419E;  /* = blue-700 */
```
...
```css
--jt-neutral-0:  #FFFFFF;  /* 흰색 — 카드·표면 배경의 기준 (필수) */
--jt-neutral-50: #F7F8F9;
--jt-neutral-100:#EEF0F2;
--jt-neutral-200:#DEE2E6;
--jt-neutral-300:#C5CBD1;
--jt-neutral-400:#9DA5AF;
--jt-neutral-500:#757F8A;
--jt-neutral-600:#5A6470;
--jt-neutral-700:#434B55;
--jt-neutral-800:#2E343B;
--jt-neutral-900:#1F2328;  /* ★ 로고색 = primary */
--jt-neutral-950:#14171A;
```
...
```css
--jt-fs-2xs:10px; --jt-fs-xs:11px; --jt-fs-sm:12px; --jt-fs-base:14px;
--jt-fs-lg:16px;  --jt-fs-xl:18px; --jt-fs-2xl:22px; --jt-fs-3xl:26px; --jt-fs-4xl:30px;
--jt-lh-tight:1.35; --jt-lh-snug:1.45; --jt-lh-normal:1.6; --jt-lh-relaxed:1.75;
```

### 3-D. 간격 스케일 (씨앗 4px 그리드 파생)

```css
--jt-space-1:4px;  --jt-space-2:8px;  --jt-space-3:12px; --jt-space-4:16px;
--jt-space-5:20px; --jt-space-6:24px; --jt-space-7:32px; --jt-space-8:40px;
--jt-space-9:48px; --jt-space-10:56px; --jt-space-11:64px; --jt-space-12:72px;
```

### 3-E. 라운드 스케일 (씨앗 8px 파생) · 그림자 · 모션

```css
--jt-r-sm:4px; --jt-r-md:8px; --jt-r-lg:16px; --jt-r-xl:24px; --jt-r-full:9999px;

--shadow-xs:0 4px 12px rgba(18,21,26,.08);
--shadow-sm:0 4px 16px rgba(18,21,26,.06);
--shadow-lg:0 20px 40px rgba(18,21,26,.15);

--jt-dur-fast:.2s; --jt-dur-base:.5s; --jt-ease-standard:cubic-bezier(.4,0,.2,1);
```
...
```css
/* 텍스트 */
--jt-color-text:            var(--jt-neutral-900);  /* 본문 */
--jt-color-text-secondary:  var(--jt-neutral-500);  /* 보조 설명 */
--jt-color-text-tertiary:   var(--jt-neutral-400);  /* 힌트·placeholder */
--jt-color-text-disabled:   var(--jt-neutral-300);

/* 배경 (표면 고도) */
--jt-color-bg-layout:     var(--jt-neutral-50);   /* 페이지 바탕 */
--jt-color-bg-container:  var(--jt-neutral-0);    /* 카드·패널·표 (=흰색) */
--jt-color-bg-elevated:   var(--jt-neutral-0);    /* 모달·드롭다운(그림자 동반) */

/* 테두리·구분 */
--jt-color-border:        var(--jt-neutral-200);
--jt-color-split:         var(--jt-neutral-100);  /* 표 행 구분선 */

/* 링크·상호작용 — charcoal이 아니라 accent(info blue) */
--jt-color-link:          var(--jt-color-accent);
--jt-color-link-hover:    var(--jt-color-accent-hover);
--jt-color-focus-ring:    var(--jt-color-accent);  /* 포커스 링·활성 탭/네비 표시 */

/* 컨트롤 기본 높이 (입력·버튼·셀렉트 통일) */
--jt-control-height:      36px;
--jt-control-height-sm:   28px;
--jt-control-height-lg:   44px;
```
...
```css
[data-theme="dark"]{
  --jt-color-text:           rgba(255,255,255,.88);
  --jt-color-text-secondary: rgba(255,255,255,.55);
  --jt-color-text-tertiary:  rgba(255,255,255,.38);
  --jt-color-bg-layout:      #0F172A;   /* slate-900 */
  --jt-color-bg-container:   #1E293B;   /* slate-800 */
  --jt-color-bg-elevated:    #273449;
  --jt-color-border:         rgba(255,255,255,.14);
  --jt-color-split:          rgba(255,255,255,.08);
  /* charcoal primary는 어두운 배경에서 안 보이므로 '먹색'을 밝게 반전(inverted ink):
     솔리드 버튼 = 밝은 표면 + 어두운 글자 */
  --jt-color-primary:        var(--jt-neutral-50);   /* 밝은 버튼 표면 */
  --jt-color-primary-text:   var(--jt-neutral-900);  /* 그 위 글자는 어둡게 */
  /* accent(파랑)는 다크 위 대비 확보용으로 밝게 */
  --jt-color-accent:         #7E9DF1;  /* = blue-300, 다크 대비 5.56 */
  --jt-color-link:           #7E9DF1;
  --jt-color-focus-ring:     #7E9DF1;
}
```
...
| **컴팩트(Compact)** | 데이터 밀집 화면(전성분 편집테이블, 견적 품목표) | `--jt-control-height` −4px, 세로 패딩 한 단계 축소, `--jt-fs-base` 유지 |

예: PLM의 전성분 편집테이블 = **다크 + 컴팩트** 조합.

> Table 헤더 배경은 라이트에서 `--jt-neutral-50`, 다크에서 `--jt-color-bg-elevated`를 씁니다.
...
| ◆/○ | 컴포넌트 | 규칙 |
|---|---|---|
| ◆ | **Button** | Primary(채움)=charcoal / Default(선)=border+text / Text=글자만 / Link=accent. 높이 `--jt-control-height`. 위험 동작은 danger(error색) |
...
| ○ | **Divider** | 구분선 `--jt-color-split` |

**③ Navigation(내비게이션)**
| ◆/○ | 컴포넌트 | 규칙 |
|---|---|---|
| ◆ | **Menu / Nav** | 활성=accent 텍스트+좌측 바, 비활성=`--jt-color-text-secondary` |
| ◆ | **Tabs** | 활성 탭 밑줄=`--jt-color-accent` |
...
| ◆ | **Input / InputNumber** | 높이 `--jt-control-height`, 포커스 링 `--jt-color-focus-ring`, placeholder=tertiary |
...
| ◆ | **Upload** | 자료업로드·MSDS 첨부. 점선 테두리 `--jt-color-border`, 호버 accent, 진행률 동반 |

**⑤ Data Display(표시)**
| ◆/○ | 컴포넌트 | 규칙 |
|---|---|---|
| ◆ | **Table** (Data Grid) | 헤더 `--jt-neutral-50`/elevated, 행 구분 `--jt-color-split`, 호버 `--jt-color-accent-bg`, 숫자열 우측정렬+`--jt-seed-font-num`(tabular-nums). 정렬·고정열 지원 |
| ◆ | **Card** | 배경 container, radius `--jt-r-lg`. 런처 타일=아이콘만 §7 색, 그 외 공통 |
